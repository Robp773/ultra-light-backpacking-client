...
export class Main extends React.Component {
  ...
  handleChoice(listName) {
    fetch(
      `${API_BASE_URL}/list-state/${this.state.activeUser}/${listName}`
    ).then(res => {
  ...
  onLogin(username) {
    ...
    const newListInput = (
      <form
        ...
        onSubmit={e => {
          e.preventDefault();
          this.handleNewList(this.createListInput.value, this.state.activeUser);
        }}
      >
        <input
          ref={input => {
            this.createListInput = input;
          }}
          className="newListInput"
          type="text"
          placeholder="Enter New List Name"
        />
        <button className="createListBtn" type="submit">
          Create
        </button>
      </form>
    );

    fetch(`${API_BASE_URL}/list-state/name-list/${this.state.activeUser}`).then(
      res => {
        res.json().then(resJSON => {
          ...
          const listNameDivs = resJSON.map((list, index) => {
            return (
              <div
                ...
                key={index}
                onClick={() => {
                  this.handleChoice(list.listName);
                }}
              >
            ...
            onClick={() => {
              this.handleDelete(
                ...
                NotificationManager.warning(
                  `${this.state.listToDelete} Deleted`
                )
              );
            }}
          >
    ...
    // otherwise display the list of lists
      ...
      return (
        <div className="containerDiv">
          ...
          {landingModal}
          <SelectList
            newListInput={this.state.newListInput}
